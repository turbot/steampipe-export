...
func getColumns(schema *proto.TableSchema) ([]string, error) {
	...
	if len(columns) == 0 {
		log.Printf("[INFO] selecting all columns")
		columns = schema.GetColumnNames()
	} else {
		tableColumns := schema.GetColumnMap()
		log.Printf("[INFO] %s tableColumns %s", strings.Join(maps.Keys(tableColumns), ","))
		for _, item := range columns {
			if _, ok := tableColumns[item]; !ok {
